...
-  the model being open source doesn't remove the threat here. "Open source" describes the weights/architecture (Gemma 3), not where inference runs. Every raw message still leaves our infrastructure to a third party's servers, subject to whatever logging/retention/subpoena exposure they have, regardless of the model being open weight. The privacy risk would only actually go away if you self-hosted that same open-source model on infrastructure you control (e.g., local Ollama or our own GPU box) — then no third party ever sees the conversation.